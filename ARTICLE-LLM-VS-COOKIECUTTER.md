...
*Suggested Reddit title: "Can an LLM generate your Django project boilerplate? Ten weeks of trying, and everything the model broke along the way."*
...
**"Improved" snippets.** One simplified line in the admin setup, and user passwords started saving as plain text. The site ran, all tests passed. The bug surfaced only when a user saved through the admin could no longer log in.

**Insecure defaults.** A docker-compose file published the database port on all interfaces instead of localhost: Postgres open to the local network, default password and all. No warning anywhere in the output.
...
**Too many add-ons to remember.** Django isn't batteries-included anymore: accounts, tasks, email, and storage are third-party choices, ten to fifteen per real project, so the scaffold has to ask which ones to include. Told "ask about add-ons" in prose, smaller models dropped whole categories, email most of all. At a minimum, we must provide the model with a comprehensive list of everything we want included in the project.

Even the newest AI models make the same mistakes—they just hide them better. Their knowledge still has an end date, and that problem never goes away. And anyway, I don't want to waste my expensive AI subscription just to write basic code. We don't need a super-smart AI to write standard templates; we just need the knowledge to sit outside the model.
...
Unlike traditional code generators, an AI doesn't give you the exact same result every time you ask. To make sure the skill really works, I created a continuous practice loop. The AI would generate a project, check it for mistakes, and figure out how to avoid those mistakes in the future. We repeated this cycle over and over to refine the instructions. Then, to prove these improvements were real, I compared the results against a 'control group'—I gave the exact same prompts to a basic AI without any of our special instructions to see how it performed on its own. Giving instructions to an AI isn't free. If you give it too much text to read, it gets confused and makes mistakes. Every single line of instruction had to fight for its place: 30–95 each, down from one 300-line document. Smaller kept winning.
