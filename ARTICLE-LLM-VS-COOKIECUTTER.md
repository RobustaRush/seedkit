...
**Insecure defaults.** Without an obvious production target, the model shipped its development settings untouched: debug mode on, the host check wide open, a throwaway secret key committed straight to the repo. Nothing warned that any of it was unsafe to deploy.
...
**Too many add-ons to remember.** Django isn't batteries-included anymore: a real project pulls in ten to fifteen third-party choices for accounts, tasks, email, and storage. Asked in prose to "figure out what's needed," the model quietly dropped whole categories, and you'd only notice when the feature that depended on one turned up missing.
