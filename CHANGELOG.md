...
- Testcase 02-shop deploy check passes `EMAIL_URL` / `DEFAULT_FROM_EMAIL` / `DJANGO_SECURE_SSL_REDIRECT=False` to the prod containers and uses a ≥ 50-char `DJANGO_SECRET_KEY`; structural assertions for Tailwind+DaisyUI sources moved to `tailwind-src/css/` and the spurious root `docker-compose.yml` requirement (dev mode is uv-on-host) dropped.
