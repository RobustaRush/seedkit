...
- `pages` app exposes `liveness` and `readiness`; `path('healthz', ...)` and `path('readyz', ...)` (no trailing slash) in `urlpatterns`. `Caddyfile` upstream block uses `health_uri /healthz` (liveness — DB blips on `/readyz` would flap the upstream). `curl http://127.0.0.1:8000/healthz` returns 200 `ok` against the local Compose stack.
...
# Caddyfile probes the liveness path
grep -q '/healthz' Caddyfile
uv run pyright
